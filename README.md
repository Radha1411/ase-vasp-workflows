...
- Post-process VASP NEB calculations using VTST output
- Generate NEB energy profiles and activation-barrier summaries
...
Post-process completed VASP NEB calculations using output generated by the VTST tools.

The workflow expects the following VTST files in the working directory:
...
```bash
python3 scripts/neb_summary.py
```
...
- reaction energy
- highest discrete NEB image
- discrete-image activation barrier
- VTST spline activation barrier
...
The NEB profile uses the reaction coordinate and spline energy reported by VTST. No additional interpolation is performed by the Python script.
...
Final energy:           -123.406789 eV
Reaction energy:        0.050000 eV
Highest discrete image: 4
Discrete barrier:       0.820000 eV
VTST spline barrier:    0.825000 eV
Spline maximum image coordinate: 3.980000
Reverse barrier:        0.775000 eV
...
The script automatically reads the number of NEB images from the VTST output and therefore does not require a fixed number of images.
